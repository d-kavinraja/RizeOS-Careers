...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY.length < 20) {
      console.log("[v0] API key missing, using fallback extraction...")
      const parsedResume = {
        personalInfo: {
        ...
        summary: "Resume uploaded and parsed successfully using pattern matching.",
      ...
      return NextResponse.json({
        ...
        note: "Parsed using pattern matching - AI parsing requires API key configuration",
      })
    }
...
function extractExperience(text: string): any[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^[A-Z][a-zA-Z\s]+(?:Engineer|Developer|Manager|Analyst|Specialist|Intern|Associate)/i)) {
      const nextLine = lines[i + 1]?.trim() || ""
      const datePattern = /\d{4}|\d{1,2}\/\d{4}|present|current/i

      experience.push({
        title: line,
        company: nextLine.match(/^[A-Z][a-zA-Z\s&.,]+/) ? nextLine : "Company details in resume",
        duration: line.match(datePattern) || nextLine.match(datePattern) ? "See resume for dates" : "Various",
    ...
    : [
        {
          ...
          duration: "Various",
          description: "Work experience as detailed in the uploaded resume",
        },
      ]
}
...
function extractEducation(text: string): any[] {
  ...
  for (const line of lines) {
    if (line.match(/bachelor|master|phd|degree|university|college|institute/i)) {
      education.push({
        degree: line.trim(),
        institution: "See resume for institution details",
        year: line.match(/\d{4}/) ? line.match(/\d{4}/)?.[0] : "See resume",
      })
      break
    }
  }
    ...
    : [
        {
          ...
          institution: "See original document",
          year: "Various",
        },
      ]
}
...
function extractCertifications(text: string): string[] {
  ...
  const certKeywords = ["certified", "certification", "certificate", "aws", "azure", "google cloud", "microsoft"]

  for (const keyword of certKeywords) {
    if (text.toLowerCase().includes(keyword)) {
      certifications.push(`${keyword.charAt(0).toUpperCase() + keyword.slice(1)} Certification`)
    }
  }
